...
import { readJson, writeJson, exists, ensureFile } from "https://deno.land/std/fs/mod.ts"
...
function injectDependenciesPlugin(changeMap: ChangeMap, outputPathMap: any, compilerOptions: CompilerOptions) {
  return async (input: string, output: string | undefined, depsDir: string) => {
    ...
    const outputText = traverseTextFile(data, compilerOptions, (node: ts.Node) => {
      // console.log(ts.SyntaxKind[node.kind]);

      const moduleNode = getImportModuleNode(node, data) || getExportModuleNode(node)
      ...
      if (node.importClause?.isTypeOnly || node.exportClause?.isTypeOnly) { return node }
      ...
      let newPath = outputPathMap[resolvedPath] = outputPathMap[resolvedPath] || `${v4.generate()}.js`
...
function changeDescription(changes: Change[]) {
  const c: { [type in ChangeType]: number } = changes.reduce((object, { type }) => {
...
export class Bundler {
  depsDir: string
  depsMapName: string
  constructor({ depsDir = "deps", depsMapName = "deps.json" }: { depsDir?: string, depsMapName?: string } = {}) {
    this.depsDir = depsDir
    this.depsMapName = depsMapName
  }
  async bundle({ input, name, dir = "dist", plugins }: Entry, { reload = false, importMap = { imports: {} }, compilerOptions = {} }: { reload?: boolean, importMap?: ImportMap, compilerOptions?: CompilerOptions } = {}) {
    ...
    const dependencyMap = (await exists(mapFilePath) && await readJson(mapFilePath) || {}) as DependencyMap

    const { changeMap, outputPathMap } = await createChangeMap(input, output, dependencyMap, { dir, depsDir: this.depsDir, reload, importMap })

    const changes = Object.values(changeMap)

    if (changes.length) {

      const injectDependencies = injectDependenciesPlugin(changeMap, outputPathMap, compilerOptions)
      ...
      for (const change of changes) {
        ...
        if (isURL(resolvedInput) || /\.(ts|js)$/.test(resolvedInput)) {
          ...
          source = await injectDependencies(resolvedInput, output, this.depsDir)
        } else {
        ...
        const outputFilePath = output ? join(depsDirPath, output) : join(depsDirPath, outputPathMap[input])

        // get local file path for url imports
        const fileInputPath = resolveURLToCacheModulePath(input) || resolvedInput

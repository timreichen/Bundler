...
async function getSplitModules(
  path: string,
  importMap: ImportMap,
): Promise<{ input: string; dependencies: Dependency[] }[]> {
  ...
  async function getDependencies(path: string, module: any) {
    ...
    for (const dependency of dependencies) {
      const resolvedPath = await resolveDependencyPath(
        path,
        dependency.path,
        importMap,
      );
      ...
      module.dependencies.push(
        { path: resolvedPath, dynamic: dependency.dynamic },
      );
    }
    return module;
  }
 ...
 * ```
 */
function createDependencyTrasformerGenerator(
  changeMap: ChangeMap,
  outputMap: OutputMap,
  dirPath: string,
  depsDirPath: string,
  ...
  return (source: string, input: string, system: boolean) => {
    ...
    function getOutputForPath({ path, dynamic }: Dependency) {
      ...
      if (system) {
        ...
        const dir = change.output
          ? join(depsDirPath, dirname(change.output))
          : depsDirPath;
    ...
    return (context: ts.TransformationContext) => {
      const visit: ts.Visitor = (node: ts.Node) => {
        const moduleNode = getImportNode(node) || getExportNode(node) ||
...
function createChangeDescription(changes: Change[]) {
  ...
  const array = Object.entries(c).map(([type, number]) =>
    blue(`${type}: ${number}`)
  ).sort();
...
export class Bundler {
  ...
  async bundle(
    ...
    const changeMap = await createChangeMap(
      path,
      output,
      deps,
    ...
    if (changes.length) {
      ...
      const createDependencyTrasformer = createDependencyTrasformerGenerator(
        changeMap,
        outputMap,
        dir,
        depsDirPath,
      ...
      const transpile = async (
        ...
        const { diagnostics, outputText } = ts.transpileModule(source, {
          ...
          transformers: {
            before: changeMap[path]
              ? [createDependencyTrasformer(source, path, toSystem)]
              : undefined,
      ...
      if (toSystem) {
        ...
        for (const change of changes) {
          ...
          deps[path] = {
            ...
            modified,
            dependencies: Object.values(dependencies).map((dependency) =>
              dependency.path
            ),
          };
        }

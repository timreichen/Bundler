...
import { exists, existsSync, writeJson, ensureFile } from "https://deno.land/std/fs/mod.ts"
...
export function createCacheModulePathForURL(url: string) {  
  ...
  return join(cachedir(), "deps", fileUrl.protocol.replace(":", ""), fileUrl.hostname, hash)
}
...
export function resolveURLToCacheModulePath(url: string) {
  ...
  return cacheModulePath && existsSync(cacheModulePath) ? cacheModulePath : null
}
 ...
 * @param path 
 */
export async function cache(path: string, reload: boolean = false) {

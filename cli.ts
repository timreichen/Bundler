...
function createOption({ alias, name, append = "", description = "" }: any) {
  const aliasName = alias ? `-${alias}, ` : " ".repeat(4)
  const longName = `--${name}`
  const indent = "\t"
  return `${indent}${aliasName}${longName} ${append || ""}\n${description.split("\n").map((line: string) => `${indent.repeat(2)}${line}`).join(`\n`)}`
}

const configOption = createOption({ name: "config", alias: "c", description: "Load config.json configuration file" })
const helpOption = createOption({ name: "help", description: "Prints help information" })
const importmapOption = createOption({ name: "importmap", append: "<FILE>", description: "UNSTABLE:\nLoad import map file\nDocs: https://deno.land/manual/linking_to_external_code/import_maps\nSpecification: https://wicg.github.io/import-maps/\nExamples: https://github.com/WICG/import-maps#the-import-map" })
const nameOption = createOption({ name: "name", alias: "n", description: "name of output file" })
const dirOption = createOption({ name: "dir", append: "<DIR>", description: "Output directory" })
const reloadOption = createOption({ name: "reload", alias: "r", description: "Reload source code (recompile all files)\n--reload Reload everything" })

async function runBundle({ input, name, dir, importmap, config, reload }: any) {

  const importMap = importmap && readJsonSync(importmap) as ImportMap
  const compilerOptions = config && (readJsonSync(config) as any).compilerOptions
...
function runHelp() {
  const name = "bundler"
  console.log("USAGE:")
  console.log(`\t${name} bundle bundle --dir dist --importmap=importmap.json --config tsconfig.json src/index.ts --name index.js`)
  console.log("\n")
  console.log("OPTIONS:")

...
export async function createOutputMap(
  ...
  const outputMap: OutputMap = {
    ...Object.values(dependencyMap).reduce((object, dependency) => {
      object[dependency.path] = join(depsDirPath, dependency.output) ||
        `${v4.generate()}.js`;
      return object;
    }, {} as OutputMap),
    ...changes.reduce((object, change) => {
      object[change.input] = join(
        depsDirPath,
        change.output || dependencyMap[change.path]?.output ||
          `${v4.generate()}.js`,
      );
      return object;
    }, {} as OutputMap),
